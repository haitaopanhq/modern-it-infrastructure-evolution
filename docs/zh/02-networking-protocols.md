...
现代网络协议演进的核心是控制面与数据面的持续分离。TCP/IP 奠定了网络通信的基础架构：IP 负责寻址和路由，TCP 提供可靠传输，DNS 和 HTTP 在上层组织应用通信。这个架构在互联网时代运转良好，但面对云原生和 AI 数据中心的需求，传统协议栈的静态特征逐渐成为瓶颈。SDN 将网络控制面从硬件交换机中剥离，实现了策略的集中编排和灵活下发。eBPF 进一步将可编程性带入内核数据面，让网络策略、安全和观测能力直接在数据包路径上执行，无需修改应用程序或引入额外代理。从 TCP/IP 的基础通信到 eBPF 的可编程数据面，协议的演进方向始终是让网络变得更灵活、可观测、可编程。

### 2.9.2 网络虚拟化演进

网络虚拟化是支撑现代数据中心弹性网络的基础。最早的技术是 VLAN 和 VXLAN：VLAN 用 802.1Q 标签在二层网络内做逻辑隔离，但最多只支持 4096 个网段，在大规模和多租户场景下很快耗尽。VXLAN 将二层帧封装到三层 UDP 包中，VNI 扩展到 1600 万，物理拓扑与逻辑网络彻底解耦。从此虚拟机、容器和 Pod 可以在任意物理位置自由迁移，IP 不变、网络不中断。

SDN 的出现进一步将控制面与数据面分离。OpenFlow 让网络策略从交换机固件中解放出来，由集中控制器统一下发。虽然 OpenFlow 本身未大规模普及，但它催生了软件化网络功能的理念。OVS（Open vSwitch）将交换机能力引入内核模块，支持 OpenFlow、VXLAN、GRE、Geneve，成为 OpenStack 和早期 K8s 网络的重要底座。

eBPF 是网络虚拟化的最新里程碑。Cilium 利用 eBPF 在内核数据面动态注入网络策略、L3/L4 安全、可观测性和负载均衡逻辑，无需修改内核代码。XDP 进一步让数据包在网卡驱动层就被处理，DPDK 则通过用户态轮询模式绕过内核协议栈实现超高 PPS。网络虚拟化的演进终点不是封装，而是可编程数据面。